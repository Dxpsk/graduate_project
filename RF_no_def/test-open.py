...
if __name__ == '__main__':
    ...
    matrix_test_datast = ['/data/Deep_fingerprint/processed_RF_data/Undefence_ow-test.npy']

    device = torch.device('cuda:5')
    # TODO: change the trained model path
    defense_model = load_model(const.num_classes_ow, '/home/xjj/projects/graduate_project/RF_no_def/model_ow', device).eval()
    ...
    for i, path in enumerate(matrix_test_datast):
        ...
        cur_website_path = '/home/xjj/projects/graduate_project/RF_no_def/result/no_def_ow.csv'
        with open(cur_website_path, 'w+', encoding='utf-8', newline='') as f:
            writer = csv.writer(f)
            for item in website_res:
                writer.writerow(item)
        pre_recall.score_func_precision_recall('/home/xjj/projects/graduate_project/RF_no_def/result/open_ana.csv', website_res, const.num_classes_ow - 1)
