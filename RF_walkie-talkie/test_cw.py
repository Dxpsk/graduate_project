...
if __name__ == '__main__':
    ...
    test_dataset = ['/data/Deep_fingerprint/processed_RF_data/walkie-talkie_cw-test.npy']
    device = torch.device('cuda:5')
    # TODO: change the trained model path
    defense_model = load_model(const.num_classes, '/home/xjj/projects/graduate_project/RF_walkie-talkie/model_cw', device).eval()
    ...
    for i, path in enumerate(test_dataset):
        ...
        cur_website_path = '/home/xjj/projects/graduate_project/RF_walkie-talkie/result/walkie-talkie_cw.csv'
        with open(cur_website_path, 'w+', encoding='utf-8', newline='') as f:
            writer = csv.writer(f)
            for item in website_res:
                writer.writerow(item)

        acc = pre_recall.pre_recCall(cur_website_path, '/home/xjj/projects/graduate_project/RF_walkie-talkie/result/walkie-talkie_cw_evaluate.csv', const.num_classes)
        print('avg acc:' + str(acc))
        #avg acc:0.9874736842105263